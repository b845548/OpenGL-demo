...
void sea_init(void) {
  ...
  SDL_Surface * t;
  int vp[4], w, h;
  static char * files[] = {"Texture/gold.jpg", 
			"Texture/land_ocean_ice_2048_glossmap.png", 
			"Texture/moon.jpg", 
			"Texture/sky3.png",
			"Texture/bois2.jpg",
			"Texture/bl2.png",
...
glBindVertexArray(_objTabVao[OBJ_LAMBORGHINI]);
  glDrawElements(GL_TRIANGLES, 3 * (_tabModel[OBJ_LAMBORGHINI]->numtriangles-3), GL_UNSIGNED_INT, 0);
gl4duPopMatrix();
 /*
...
if(SDL_GetTicks()-timer>=100)dist+=45*dt;

gl4duPushMatrix();
gl4duTranslatef(0,0,-20);
gl4duRotatef(15,0,1,0);
if(SDL_GetTicks()-timer>=100)rot+=50*dt;
...
for(freeMemory=0;freeMemory<OBJNB-1;freeMemory++){
